...
const itemIds = [
  ...
  // Рукавиці / Щити / Кільця / Амулети
  1648, 1651,
  // Нагрудники / Обладунки / Мантії
  ...
  // Стріли / Амулети / Магічні предмети
  2154, 2155, 2156, 2157, 2158, 2159
];

// Функція автоматичного визначення слоту та характеристик за ID
function generateArtifact(id) {
  const fullId = `fc${id}`;
  const iconPath = `assets/items/${fullId}.png`;
  
  let slot = 'ring';      // Дефолтний слот
  let name = `Mystic Artifact #${id}`;
  let bonusDamage = 0;
  let bonusHP = 0;

  // Визначаємо тип предмету за його числовим ID
  if (id <= 1603 || (id >= 1677 && id <= 1689)) {
    slot = 'weapon';
    name = `Vanguard Weapon #${id}`;
    bonusDamage = Math.floor((id % 10) + 5); // Динамічний урон від 5 до 14
  ...
  // Створення красивого опису предмета
  let description = '';
